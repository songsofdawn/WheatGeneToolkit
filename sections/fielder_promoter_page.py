...
from utils.db_query import get_fielder_promoters_many
...
def render():
    ...
    if st.button("开始抓取 Fielder 启动子", key="btn_fielder_promoter"):
        ...
        with st.spinner("正在批量提取 Fielder 启动子序列，请稍候..."):
            all_promoter_df = get_fielder_promoters_many(tuple(gene_ids))
        ...
        show_dataframe_preview(summary_df, label="Fielder 启动子统计结果", key="show_all_fielder_promoter_summary")
        if fasta_records:
            st.download_button(
